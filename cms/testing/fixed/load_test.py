...
class LoadTester:
    ...
    async def run_load_test(self, concurrent_users: int, duration_seconds: int, 
        ...
        if successful_requests > 0:
            ...
            p95_response_time = sorted_results[min(p95_index, len(sorted_results) - 1)]
            p99_response_time = sorted_results[min(p99_index, len(sorted_results) - 1)]
        else:
...
async def test_authentication_load():
    ...
    for i in range(100):
        scenarios.append({
            ...
            'data': {
                'username': f'loadtest_user_{i}_{random.randint(10000, 99999)}',
                'email': f'loadtest_{i}_{random.randint(10000, 99999)}@example.com',
...
async def test_database_operations_load():
    ...
    async with aiohttp.ClientSession(connector=connector, timeout=timeout) as session:
        ...
        user_data = {
            'username': f'loadtest_dbuser_{random.randint(10000, 99999)}',
            'email': f'loadtest_db_{random.randint(10000, 99999)}@example.com',
            'password': 'loadtest123'
        }
    ...
    scenarios = [
        # Create posts
        {
            ...
            'headers': headers,
            'data': {
                'title': f'Load Test Post {random.randint(10000, 99999)}',
...
async def test_mixed_workload():
    ...
    async with aiohttp.ClientSession(connector=connector, timeout=timeout) as session:
        for i in range(10):
            user_data = {
                'username': f'mixedtest_user_{i}_{random.randint(10000, 99999)}',
                'email': f'mixedtest_{i}_{random.randint(10000, 99999)}@example.com',
            ...
            try:
                ...
                print(f"Failed to create test user {i}: {e}")
    
    if not tokens:
        print("Failed to create any test users for mixed workload test")
        return
    ...
    for token in tokens:
        ...
        scenarios.extend([
            ...
            # Write operations (20%)
            {
                ...
                'headers': headers,
                'data': {
                    'title': f'Mixed Workload Post {random.randint(10000, 99999)}',
